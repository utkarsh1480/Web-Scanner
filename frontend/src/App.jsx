...
function App() {
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col">
      {/* Navigation */}
      <nav className="bg-black/25 backdrop-blur-md border-b border-white/10 sticky top-0 z-50">
        <div className="container-custom">
          <div className="flex items-center justify-between h-20">
            <div className="flex items-center">
              <Link to="/" className="hover:opacity-90 transition-opacity flex flex-col items-start leading-none text-white">
                <span className="text-2xl font-bold tracking-tight text-white">{`{web scanner}`}</span>
                <span className="text-[9px] uppercase tracking-widest text-white/60 font-semibold mt-0.5">performance rules</span>
              </Link>
            </div>

            {/* Desktop Navigation */}
            <div className="hidden md:flex items-center space-x-8">
              <Link to="/" className="text-white/80 hover:text-white transition-colors duration-200 font-medium">{t.navHome}</Link>
              <Link to="/lighthouse" className="text-white/80 hover:text-white transition-colors duration-200 font-medium">{t.navLighthouse}</Link>
              <Link to="/github/dashboard" className="text-white/80 hover:text-white transition-colors duration-200 font-medium flex items-center gap-2">
              ...
              <Link to="/cms" className="text-white/80 hover:text-white transition-colors duration-200 font-medium">{t.navServices}</Link>
              <Link to="/contact" className="text-white/80 hover:text-white transition-colors duration-200 font-medium">{t.navContact}</Link>
              <Select value={language} onValueChange={setLanguage}>
                <SelectTrigger className="w-[140px] bg-black/40 border-white/20 text-white backdrop-blur-sm">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              {user ? (
                <div className="flex items-center space-x-4 border-l pl-6 border-white/20">
                  <Link to="/profile" className="flex items-center gap-2 hover:opacity-85 transition-opacity">
                    <img src={user.avatar} alt="Avatar" className="w-8 h-8 rounded-full border border-white/20 object-cover" />
                    <span className="text-sm font-semibold max-w-[100px] truncate text-white">{user.username}</span>
                  </Link>
                  <button onClick={logout} className="bg-white/10 text-white hover:bg-white/20 border border-white/20 text-xs px-3.5 py-2 rounded-full transition-all">
                    Logout
                  </button>
                </div>
              ) : (
                <div className="flex items-center space-x-4 border-l pl-6 border-white/20">
                  <Link to="/login" className="text-sm font-semibold hover:text-white text-white/80 transition-colors">Sign In</Link>
                  <Link to="/register" className="bg-white text-zinc-950 hover:bg-zinc-100 font-bold text-xs px-5 py-2.5 rounded-full shadow-md transition-all">Get Started</Link>
                </div>
              )}
            </div>
