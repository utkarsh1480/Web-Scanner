...
const ForgotPassword = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // For development testing ease, expose the URL if it returns it
            if (res.resetUrl) {
            ...
            <Card className="w-full max-w-md bg-card/60 backdrop-blur-xl border border-border/80 shadow-2xl shadow-primary/5">
                ...
                <form onSubmit={handleSubmit}>
                    <CardContent className="space-y-4">
                        ...
                        {success && (
                            <div className="p-3.5 rounded-lg border border-green-500/20 bg-green-500/10 text-green-600 dark:text-green-400 text-sm font-medium animate-in fade-in duration-300">
                                {success}
                            </div>
                        )}

                        {devResetUrl && (
                            <div className="p-3.5 rounded-lg border border-primary/20 bg-primary/5 text-sm space-y-2 animate-in slide-in-from-bottom-2 duration-300">
                                <p className="font-semibold text-xs text-primary uppercase tracking-wider">Development Link:</p>
                                <p className="text-xs text-muted-foreground break-all bg-background/80 p-2 rounded border border-border select-all font-mono">
                                    {devResetUrl}
                                </p>
                                <a 
                                    href={`/reset-password/${devResetUrl.substring(devResetUrl.lastIndexOf('/') + 1)}`}
